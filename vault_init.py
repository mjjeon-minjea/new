...
def init_vault():
    ...
    # 2. SCHEMA.md 생성 (LLM Wiki 지침 및 프롬프트 명세)
    schema_file = wiki_dir / "SCHEMA.md"
    schema_content = """# Obsidian LLM Wiki Schema & Guidelines
...
"""
    ...
    print(f"[+] SCHEMA.md 생성 완료: {schema_file}")
...
## 🪙 디지털 자산 및 가상자산
- [[Bitcoin]] - 비트코인 시세, 규제, 기관 수급 및 주요 온체인 분석 지표 흐름

## 🌍 글로벌 거시경제
- [[US-Fed]] - 미 연방준비제도(Fed) 통화정책, 금리 기조, FOMC 성명 및 지표 전망
- [[Global-Market]] - 미국 증시, 원자재, 환율 등 세계 금융시장 동향

## 🇰🇷 국내 경제 및 금융
- [[Korea-Economy]] - 국내 기준금리, 수출입 동향, 코스피/코스닥 지수 및 거시경제 지표
"""
    with open(index_file, "w", encoding="utf-8") as f:
        f.write(index_content.strip() + "\n")
    print(f"[+] index.md 생성 완료: {index_file}")
    
    # 4. log.md 생성
    log_file = wiki_dir / "log.md"
    log_content = """# Chronological Activity Log
...
"""
    ...
    print(f"[+] log.md 생성 완료: {log_file}")
    
    print("\n[검증 완료] Obsidian Vault 구조 생성 성공.")
